        ...
        <form method="POST" action="{{ route('alumkit.educations.update', $education) }}">
            ...
            <div class="space-y-4" x-data="{ is_current: {{ old('is_current', $education->is_current) ? 'true' : 'false' }} }">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <x-alumkit::year-select name="start_year" :label="__('alumkit::education.start_year')" :value="$education->start_year" required />
                    <x-alumkit::month-select name="start_month" :label="__('alumkit::education.start_month')" :value="$education->start_month" />
                </div>
                ...
                <div class="grid grid-cols-2 gap-4" x-show="!is_current">
                    <x-alumkit::year-select name="end_year" :label="__('alumkit::education.end_year')" :value="$education->end_year" />
                    <x-alumkit::month-select name="end_month" :label="__('alumkit::education.end_month')" :value="$education->end_month" />
                </div>
            </div>
