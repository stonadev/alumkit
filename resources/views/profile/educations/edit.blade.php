    ...
    <x-card>
        ...
        <form method="POST" action="{{ route('alumkit.profile.educations.update', $education) }}">
            ...
            <div class="space-y-4" x-data="{ is_current: {{ old('is_current', $education->is_current) ? 'true' : 'false' }} }">
                ...
                <x-alumkit::suggest name="subject" :label="__('alumkit::education.subject')" :value="old('subject', $education->subject)" :suggestions="config('alumkit.education.subjects', [])" required />
                <x-input type="text" name="student_id" :label="__('alumkit::education.student_id')" :value="old('student_id', $education->student_id)" />

                <div class="grid grid-cols-2 gap-4">
                    <x-alumkit::year-select name="start_year" :label="__('alumkit::education.start_year')" :value="old('start_year', $education->start_year)" required />
                    <x-alumkit::month-select name="start_month" :label="__('alumkit::education.start_month')" :value="old('start_month', $education->start_month)" />
                </div>
                ...
                <div class="grid grid-cols-2 gap-4" x-show="!is_current">
                    <x-alumkit::year-select name="end_year" :label="__('alumkit::education.end_year')" :value="old('end_year', $education->end_year)" />
                    <x-alumkit::month-select name="end_month" :label="__('alumkit::education.end_month')" :value="old('end_month', $education->end_month)" />
                </div>
            </div>
