        ...
        <form method="POST" action="{{ route('alumkit.careers.update', $career) }}">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4" x-data="{ is_current: {{ $career->is_current ? 'true' : 'false' }} }">
                    <x-alumkit::year-select name="start_year" :label="__('alumkit::career.start_year')" :value="$career->start_year" required />
                    <x-alumkit::month-select name="start_month" :label="__('alumkit::career.start_month')" :value="$career->start_month" />
                </div>
                ...
                <div class="grid grid-cols-2 gap-4" x-show="!is_current">
                    <x-alumkit::year-select name="end_year" :label="__('alumkit::career.end_year')" :value="$career->end_year" />
                    <x-alumkit::month-select name="end_month" :label="__('alumkit::career.end_month')" :value="$career->end_month" />
                </div>
