        ...
        <form method="POST" action="{{ route('alumkit.careers.update', $career) }}">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-800 dark:border-gray-700 dark:text-white">{{ $career->description }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
