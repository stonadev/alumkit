...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-navy">Edit Global: {{ $key }}</h1>
        <a href="{{ route('alumkit.globals.index') }}" class="text-gray-600 hover:text-navy">
            Back to Globals
        </a>
    </div>

    <form method="POST" action="{{ route('alumkit.globals.update', $key) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <x-card>
            <div class="space-y-4">
                @php
                    $fieldValues = $contents ? $contents->fields : [];
                @endphp
